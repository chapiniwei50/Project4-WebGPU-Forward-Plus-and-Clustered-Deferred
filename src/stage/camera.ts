...
class CameraUniforms {
    readonly buffer = new ArrayBuffer(64 * 4); // Increased size for additional matrices
    ...
    set invProjMat(mat: Float32Array) {
        this.floatView.set(mat, 16); // Starts after the first mat4 (16 floats)
    }

    set screenDimensions(dimensions: { width: number, height: number }) {
        this.floatView[32] = dimensions.width;  // screenWidth
        this.floatView[33] = dimensions.height; // screenHeight
    }
}
...
export class Camera {
    ...
    onFrame(deltaTime: number) {
        ...
        this.uniforms.invProjMat = invProjMat as Float32Array;
        this.uniforms.screenDimensions = {
