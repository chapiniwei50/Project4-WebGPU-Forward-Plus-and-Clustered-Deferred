...
export class Lights {
    ...
    constructor(camera: Camera) {
        ...
        // Cluster buffer
        const totalClusters = 16 * 9 * 24; // clusterCountX * clusterCountY * clusterCountZ
        const maxLightsPerCluster = 100;
        const clusterSize = 4 + (4 * maxLightsPerCluster); // u32 lightCount + u32[maxLightsPerCluster] lightIndices

        this.clusterSetStorageBuffer = device.createBuffer({
            label: "cluster set",
            size: totalClusters * clusterSize,
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
        });
    ...
    doLightClustering(encoder: GPUCommandEncoder) {
        ...
        computePass.dispatchWorkgroups(1, 1, 24);
