...
fn getClusterIndex(fragPos: vec4f) -> u32 {
    ...
    let screenPos = fragPos.xy / fragPos.w;
    let screenX = (screenPos.x * 0.5 + 0.5) * camera.screenWidth;
    let screenY = (1.0 - (screenPos.y * 0.5 + 0.5)) * camera.screenHeight;
...
@fragment
fn main(in: FragmentInput, @builtin(position) fragPos: vec4f) -> @location(0) vec4f
{
